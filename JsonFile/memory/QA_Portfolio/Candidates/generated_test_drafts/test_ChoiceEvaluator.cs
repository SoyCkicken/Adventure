...
public class TestChoiceEvaluator
{
    ...
    {
        // Explicit oracle: Clamp01(3.14f) = 1f → "clampedRate >= 1f" 분기에서
        // Random.value를 거치지 않고 결정적으로 true 반환.
    ...
    {
        // Explicit oracle: 상한 경계값 1.0f 자체도 "clampedRate >= 1f"에 포함되어
        // 결정적으로 true를 반환함을 확인한다(경계값 포함 여부 검증).
        var result = ChoiceEvaluator.EvaluateSuccess(1.0f);
        Assert.That(result, Is.EqualTo(true));
    }
    ...
    {
        // Explicit oracle: Clamp01(0f) = 0f → "clampedRate <= 0f" 분기에서
        // Random.value를 거치지 않고 결정적으로 false 반환.
