...
public class ItemManager : MonoBehaviour
{
    ...
    void DisplayWeapons()
    {
        ...
        foreach (var w in jsonManager.Weapon_Masters)
        {
            ...
            {
                var o = jsonManager.Item_Options.FirstOrDefault(x => x.Option_ID == w.Option_1_ID);
                if (o != null) myOpts.Add($"{o.Option_Description} +{w.Option_Value1}");
            }
            if (!string.IsNullOrEmpty(w.Option_2_ID))
            {
                var o = jsonManager.Item_Options.FirstOrDefault(x => x.Option_ID == w.Option_2_ID);
    ...
    {
        //Clear(weaponsParent);
        foreach (var a in jsonManager.Armor_Master)
        {
            ...
            {
                var o = jsonManager.Item_Options.FirstOrDefault(x => x.Option_ID == a.Armor_Option1);
                if (o != null) myOpts.Add($"{o.Option_Description} +{a.Option1_Value}");
            }
            if (!string.IsNullOrEmpty(a.Armor_Option2))
            {
                var o = jsonManager.Item_Options.FirstOrDefault(x => x.Option_ID == a.Armor_Option2);
    ...
    {
        //Clear(weaponsParent);
        foreach (var it in jsonManager.Item_Master)
        {
            ...
            {
                var o = jsonManager.Item_Options.FirstOrDefault(x => x.Option_ID == it.Item_Option1);
                if (o != null) myOpts.Add($"{o.Option_Description} +{it.Option1_Value}");
            }
            if (!string.IsNullOrEmpty(it.Item_Option2))
            {
                var o = jsonManager.Item_Options.FirstOrDefault(x => x.Option_ID == it.Item_Option2);
