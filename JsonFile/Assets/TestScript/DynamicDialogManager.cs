...
public class DynamicDialogManager : MonoBehaviour
{
    ...
    public GameObject TextPrefab;           //임시로 담을 TEXT프리팹
    public GameObject ImagePrefab;          //임시로 담을 이미지 프리팹
    ...
    private void HandleMain(Script_Master_Main ev)
    {
        ...
        if (blocks.Count == 0 || isImage || blocks[blocks.Count - 1].imageComp.gameObject.activeSelf)
        {
            ...
            if (isImage)
            {
                var go = Instantiate(ImagePrefab, contentParent);
                Testblocks.Add(go);
                RectTransform rt = go.GetComponent<RectTransform>();
                // 이미지 띄우고 텍스트 숨김
                rt.sizeDelta = new Vector2(700, 350);
            ...
            {
                var go = Instantiate(TextPrefab, contentParent);
                Testblocks.Add(go);
                RectTransform rt = go.GetComponent<RectTransform>();
                // 텍스트 블록 빈 상태로 시작
                ...
                // 첫 글자 찍기 코루틴 실행
                StartCoroutine(TypeText(go.GetComponent<TMP_Text>(), ev.KOR));
            }
            //Debug.Log("출력성공");
        }
        else
